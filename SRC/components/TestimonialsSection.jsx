      ...
      <div className="rounded-[24px] border border-emerald-300/15 bg-[#0b1019] p-4">
        <div className="flex items-center justify-between border-b border-white/8 pb-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-300">
              Espaço reservado para o print {whatsappProofAssetName}
            </p>
          </div>
        ...
        <div className="mt-4 rounded-[20px] border border-white/8 bg-[#111b21] p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-white">Anderson • CasaLúmina Outlet</p>
              <p className="text-xs text-slate-400">Feedback via WhatsApp</p>
            </div>
          </div>

          <div className="mt-4 ml-auto max-w-[88%] rounded-[20px] rounded-tr-sm bg-emerald-500/15 px-4 py-3 text-sm leading-7 text-slate-100 shadow-2xl shadow-emerald-500/10">
            Aqui entra o print real do WhatsApp para reforçar overdelivery, confiança e fechamento do segundo projeto.
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <section id="prova-social" className="section-panel section-panel-muted px-6 py-12 sm:px-8 lg:px-12 lg:py-14">
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2">
        {testimonials.map(({ body, icon: Icon, name, quoteIcon: QuoteIcon, role, highlight }, index) => (
          <motion.article key={name} {...reveal(0.08 + index * 0.08)} className="testimonial-card">
            ...
            {highlight ? <div className="mt-6"><WhatsappProofMockup /></div> : null}
